...
std::ostream& operator << (std::ostream &print, const Company &obj) {
    print << "This Company has " << obj.boss->get_numberOfEmplyee();
...
void Company::set_employee(Employee **employee) {
    this->employee = new Employee*[boss->get_numberOfEmplyee()];
    for (int i = 0; i < boss->get_numberOfEmplyee(); i++) {
...
Employee Company::maxEfficiency() {
    ...
    int num_employee = 0;
    for (int i = 1; i < boss->get_numberOfEmplyee(); i++) {
...
double Company::averageEfficiency() {
    ...
    return sum / boss->get_numberOfEmplyee();
}
